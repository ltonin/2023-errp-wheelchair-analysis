...
function [com, index] = errp_pop_viewprops( EEG, typecomp, chanorcomp, spec_opt, erp_opt, scroll_event, classifier_name, fig)
...
if ~exist('fig','var')
    ...
    if ~typecomp && isfield(EEG.etc, 'ic_classification')
    	set(gcf,'Position', [pos(1) 20 1024/7*column 768/5*rows*1.2]);
    else
    	set(gcf,'Position', [pos(1) 20 1024/7*column 768/5*rows]);
    end
    incx = 120;
    incy = 110;
    sizewx = 100/column;
...
closebtn = uicontrol(gcf, 'Style', 'pushbutton', 'String', 'Close', 'Units','Normalized', 'Position',...
                           [0.4 0 0.2 0.05]);

set( closebtn, 'callback', {@closewin, gcf});
...
function get_component_idx(hObject,eventData, idx)
    global index;

    if(ismember(idx, index) == true)
        index = setdiff(index, idx);
    else
        index = union(idx, index);
    end
end

function closewin(hObject, eventData, h)
    close(h);
end
